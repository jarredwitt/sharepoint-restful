...
*/
SharePoint.prototype.getListInfo = function(list, props){
    if(!props){
...
SharePoint.prototype.updateListItem = function(list, itemId, item){
    var url = this.root + "lists/getbytitle('" + list + "')/items("+ itemId + ")";

    var config = this;
    config.headers["IF-MATCH"] = "*";
    config.headers["X-HTTP-Method"] = "MERGE";

    return httpPost(url, config, item, false);
}
